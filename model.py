...
class MinimalRNNCell(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, batch_first=True, dropout=0.):
        super(MinimalRNNCell, self).__init__()
        latent_size = hidden_size

        # 定义输入层的参数
        self.input_layer = nn.Linear(input_size, latent_size, device='cuda')
        ...
        self.gate_layer = nn.Linear(latent_size + hidden_size, hidden_size, device='cuda')

        # 定义输出层的参数
        self.output_layer = nn.Linear(hidden_size, output_size, device='cuda')
